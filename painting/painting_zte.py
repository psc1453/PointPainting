...
DATASET_PATH = "../detector/data/zte/"
TWO_CAMERAS = False
...
class Painter:
    def __init__(self, seg_net_index):
        self.root_split_path = DATASET_PATH
    ...
    def get_calib_from_file_zte(self):
        calib_file = join(DATASET_PATH, 'cali_2.txt')
        ...
        Tr_cam_to_velo = np.array(obj, dtype=np.float32)
        calib = {'P2': P2.reshape(3, 4),
                 'R0': R0.reshape(3, 3),
                 'Tr_cam2velo': Tr_cam_to_velo.reshape(4, 4)}
    ...
    def lidar_to_cam(self, pointcloud, proj_mats):
        lidar_velo_coords = copy.deepcopy(pointcloud)
        reflectances = copy.deepcopy(lidar_velo_coords[:, -1])
        lidar_velo_coords[:, -1] = 1
        cam2velo = proj_mats['Tr_cam2velo']
        invR = cam2velo[:3, :3].transpose()  # Orthogonal matrix
        invT = -invR.dot(cam2velo[:3, 3])
        velo2cam = np.zeros([4, 4], dtype=cam2velo.dtype)
        velo2cam[:3, :3], velo2cam[:3, 3] = invR, invT
        velo2cam[3, 3] = 1.0
        lidar_cam_coords = velo2cam.dot(lidar_velo_coords.transpose())
    ...
    def augment_lidar_class_scores(self, class_scores_l, lidar_raw, projection_mats):
        ...
        ################################
        lidar_cam_coords = self.lidar_to_cam(lidar_raw, projection_mats)
